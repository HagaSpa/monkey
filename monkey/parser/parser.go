...
func New(l *lexer.Lexer) *Parser {
	...
	p.infixParserFn = make(map[token.TokenType]infixParserFn)
...
func (p *Parser) parseExpression(pracedence int) ast.Expression {
	...
	for !p.peekTokenIs(token.SEMICOLON) && pracedence < p.peekPrecedence() {
...
func (p *Parser) parseGroupedExpression() ast.Expression {
	//「(」を消費する
	p.nextToken()
	// 「(」から「)」までの文字を消費する。parsePrefixExpression or parseInfixExpression
